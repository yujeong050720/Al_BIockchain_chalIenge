...
현재는 **로컬 실행본**입니다. AI는 같은 PC의 별도 프로세스에서 실행되며, 공개 AI 서버나 웹 입력 화면은 포함하지 않습니다. 모델은 두 가지 가상 처방 문장을 학습한 조건부 토큰 모델이며 범용 LLM 또는 실제 의료용 모델이 아닙니다.
...
- 공개 서비스 배포, 병원·환자별 접근 권한, 다운로드·복호화 API 및 다중 프로세스 거래 조정은 별도 구현이 필요합니다.
